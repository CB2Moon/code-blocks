...
/**
 * De-duplicate delimiter selections while preserving order.
 */
...
function updateTargetHighlights(editor: vscode.TextEditor, vscodeSelection: vscode.Selection): void {
    ...
    if (previous) targets.push(previous.toVscodeSelection());
    if (next) targets.push(next.toVscodeSelection());
    if ((!next || !previous) && parent) forceTargets.push(parent.toVscodeSelection());
